...
class DonationCommand
  ...
  def execute
    return "기부 금액은 1 이상으로 입력해주세요.\n예: [기부/30]" if @amount <= 0

    user = @sheet_manager.find_user(@sender)
    return "아직 등록되지 않은 계정입니다." unless user

    house = user[:house].to_s.strip
    return "사용자 기숙사 정보가 없습니다. 사용자 탭의 기숙사를 확인해주세요." if house.empty?
    ...
    unless new_house_score
      ...
      return "기숙사 점수 반영 중 오류가 발생했습니다. 기숙사 탭의 기숙사명을 확인해주세요."
    end
